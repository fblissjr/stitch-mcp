...
- Level is seeded from `STITCH_MCP_LOG_LEVEL` (or legacy `STITCH_MCP_LOG=1` -> `full`). Default log dir is `<project-root>/.stitch-mcp/log`, resolved from the module location via `resolveDefaultLogRoot()` (walks up to the nearest `package.json`) so it doesn't depend on the launch CWD; override with `STITCH_MCP_LOG_DIR`. Runtime changes go through the `set_log_level` virtual tool, which is intentionally agent-callable (any connected MCP client can change verbosity) -- note this also means the model can escalate what gets persisted to disk.
